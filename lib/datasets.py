...
class CIFAR10(Dataset):
    def __init__(self, model, test_batch_size, augment, split_size, randomize_labels):
        ...
        self.trainset = lib.cifar.CIFAR10(
            root="./data",
            train=True,
            download=False,
...
class MNIST(Dataset):
    def __init__(self, device, split_size, test_batch_size):
        ...
        self.unnormalizer = transforms.Compose(
            [
                transforms.Normalize(mean=[0.0, 0.0, 0.0], std=[1 / 0.3081]),
                transforms.Normalize(mean=[-0.1307], std=[1.0, 1.0, 1.0]),
            ]
        )
...
class SVHN(Dataset):
    def __init__(self, model, test_batch_size, split_size, augment):
        ...
        testset = IndexedSVHN(
            root="./svhn_data", split="test", download=False, transform=transform_test
        )
        self.testloader = torch.utils.data.DataLoader(
        ...
        self.trainset1 = IndexedSVHN(
            root="./svhn_data", split="train", download=False, transform=transform_train
        )
        self.trainset2 = IndexedSVHN(
            root="./svhn_data", split="extra", download=False, transform=transform_train
        )
...
class ImageNet(Dataset):
    def __init__(self, model, test_batch_size, traindir, valdir, split_size):
        ...
        self.trainset = IndexedImageFolder(traindir, transform_train)
        self.num_training_images = len(self.trainset)
