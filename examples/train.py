...
parser.add_argument(
    "--hours", type=float, default=12, help="number of hours to train (default: 12)"
)
...
parser.add_argument("--output_dir", default="./logs", help="directory to place logs")

parser.add_argument("--prob_pow", type=float, default=3, help="dictates SB selectivity")
parser.add_argument(
    "--staleness",
    type=int,
    default=3,
...
parser.add_argument(
    "--lr_sched", default=None, help="path to file with manual lr schedule"
)
parser.add_argument(
    "--forwardlr",
    dest="forwardlr",
...
if args.dataset == "cifar10":
    num_classes = 10
    train_dataset = dataset_lib.CIFAR10(
        root="data/", train=True, transform=train_transform, download=True
    )

    test_dataset = dataset_lib.CIFAR10(
        root="data/", train=False, transform=test_transform, download=True
    )
elif args.dataset == "cifar100":
    num_classes = 100
    train_dataset = dataset_lib.CIFAR100(
        root="data/", train=True, transform=train_transform, download=True
    )

    test_dataset = dataset_lib.CIFAR100(
        root="data/", train=False, transform=test_transform, download=True
    )
elif args.dataset == "svhn":
    num_classes = 10
    train_dataset = dataset_lib.SVHN(
        root="data/", split="train", transform=train_transform, download=True
    )

    extra_dataset = dataset_lib.SVHN(
    ...
    test_dataset = dataset_lib.SVHN(
        root="/ssd/datasets/svhn/",
        split="test",
...
while time.time() - start_time_seconds < args.hours * 3600.0:
    epoch += 1
